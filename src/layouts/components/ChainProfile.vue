  ...
  <div class="dropdown">
    <label tabindex="0" class="flex items-center">
      <div class="p-1 relative mr-3 cursor-pointer">
        <img v-lazy="chainStore.logo" class="w-9 h-9 rounded-full bg-white border border-white" />
      ...
      <div class="flex-1 w-0">
        <div :key="baseStore.latest?.block?.header?.height ||
    ...
    <div tabindex="0" class="dropdown-content flex-row left-6 w-80 menu shadow bg-base-200 rounded-box overflow-auto">
      <!-- rest -->
      ...
      <!-- rest -->
      <div class="px-4 py-2 text-sm text-gray-400 text-bold" v-if="chainStore.current?.endpoints?.rpc">
        RPC Endpoint
      </div>
      <div v-for="(item, index) in chainStore.current?.endpoints?.rpc"
      ...
      <input v-model="rpcModel" class="input w-full !input-bordered input-sm" @keydown.enter.prevent="
